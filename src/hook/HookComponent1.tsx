...
// - useEffect : 컴포넌트의 생명주기에 따라 특정 작업을 수행할 수 있도록 하는 함수 mount, update, unmount 해당하는 생명주기에 따라 실행
...
// - hook 함수는 반드시 컴포넌트 코드 블럭 라인에 존재해야 한다.
...
export default function HookComponent1() {
    ...
    // - useEffect(호출할 콜백 함수, 스코프 할 상태 배열);
    ...
    // 스코프 할 상태 배열에 상태를 지정하면 지정한 상태가 바뀔 때 마다 함수가 호출됨
    useEffect(() => {
        // 스코프 할 상태 배열에 지정한 상태를 해당 effect에서 변경하면 무한 호출 됨
        // setState1(state1 + 1);
        //console.log('state1 상태 변경!')
    }, [state1]);


    // 스코프 할 상태 배열에 두 개 이상의 상태를 지정할 수 있다.
...
function SubComponent () {
    ...
    // 전달한 콜백 함수의 return 으로 컴포넌트가 umount 될 때 실행할 함수를 반환
