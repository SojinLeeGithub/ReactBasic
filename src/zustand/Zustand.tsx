...
// zustand 패키지:
// - react에서 사용할 수 있는 상태관리 패키지 중 하나
// - 상태관리 패키지 중 과거에는 Redux 패키지가 가장 많이 사용되었지만 현재는 zustand로 이동중이다.
...
// - zustand 패키지에 있는 create 함수로 상태 store를 생성할 수 있다.
// - store: 관리할 상태들과 각 상태에 해당하는 상태 변경 함수들의 집합
// - create 함수를 호출하면 store를 반환하는 hook 함수를 반환한다.
...
// - typescript에서는 zustand의 store타입을 create 함수의 제너릭으로 전달해줘야함
...
// style={{height: '400px', backgroundColor: 'grey'}}
...
function Sub1Component2() {
     return (
    <div style={{marginTop: '40px', height: '100px', backgroundColor: 'pink'}}>
         <Sub2Component2 />
    </div>
    )
}
...
function Sub2Component1() {
    ...
    return (
        <div style={{height: '75px', backgroundColor: 'skyblue'}}>
